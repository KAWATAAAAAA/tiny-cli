...
const appVersion = require(`${path.resolve(process.cwd(),"package.json")}`).version
const fs = require("fs")
...
const defaultInject = () => {
    return "/* @App version: " + appVersion + " " + "@Build Date: " + buildDate() + "*/"
}
        ...
        writeBundle:async () => {
            setImmediate(async () => {
                fs.mkdir(filePath, { recursive: true },(err:any) => {
                    !err && fs.writeFile( filePath +"/"+ fileName, content,{ flag: "a" }, (err:any) => {
                        if(err){
                            console.log(err)
                        }
                    })
...
function findAssetFileInjectCode(assetInfo:any,ext:string,injectData:string | Function,position:string){
