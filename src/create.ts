...
const fetchRepoList = async (): Promise<Array<any>> => {
  const { data } = await axios.get('https://api.github.com/orgs/Tiny-CLI-Template/repos');
  return data;
};
// 2.获取项目的tag列表
...
// 3.封装执行函数时的loading效果
const waitFnloading = (fn: Executor, message: LoadingMsg) => async (...args: Array<any>) => {
...
// 前置目录检查
const beforeCreate = async (projectName: AppName): Promise<FileActionsResult> => {
